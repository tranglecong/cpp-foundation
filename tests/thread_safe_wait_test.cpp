...
TEST(WaitTest, WaitForWithPredicateAndTimeoutTest) {
    ...
    auto status = w.waitFor(std::chrono::milliseconds(100), [&]() -> bool { return predCalled.load(); });
    EXPECT_EQ(status, Wait::Status::SUCCESS);
    predTrigger.join();
}
...
TEST(WaitTest, WaitForPredicateTimeoutTest) {
    Wait w;
    auto status = w.waitFor(std::chrono::milliseconds(100), []() -> bool { return false; });
    EXPECT_EQ(status, Wait::Status::TIMEOUT);
}
...
class MultithreadWaitTest : public ::testing::Test {
  protected:
    ...
    void runNotifyTest(std::function<void()> notifyFunc) {
        ...
        std::thread t1([&]() { EXPECT_EQ(w.wait(), Wait::Status::SUCCESS); });
        std::thread t2(
            [&]() { EXPECT_EQ(w.wait([this]() -> bool { return predCalled.load(); }), Wait::Status::SUCCESS); });
        std::thread t3([&]() { EXPECT_EQ(w.waitFor(std::chrono::milliseconds(200)), Wait::Status::SUCCESS); });
        std::thread t4([&]() {
            EXPECT_EQ(w.waitFor(std::chrono::milliseconds(200), [this]() -> bool { return predCalled.load(); }),
                      Wait::Status::SUCCESS);
        });
...
TEST_F(MultithreadWaitTest, TimeoutTest) {
    // Test with timeout
    std::thread t1([&]() { EXPECT_EQ(w.waitFor(std::chrono::milliseconds(100)), Wait::Status::TIMEOUT); });
    std::thread t2([&]() {
        EXPECT_EQ(w.waitFor(std::chrono::milliseconds(100), [this]() -> bool { return predCalled.load(); }),
                  Wait::Status::TIMEOUT);
    });
...
TEST_F(MultithreadWaitTest, ExitTest) {
    ...
    std::thread t1([&]() { EXPECT_EQ(w.wait(), Wait::Status::EXIT); });
    std::thread t2([&]() { EXPECT_EQ(w.waitFor(std::chrono::milliseconds(200)), Wait::Status::EXIT); });
    std::thread t3([&]() {
        EXPECT_EQ(w.waitFor(std::chrono::milliseconds(200), [this]() -> bool { return predCalled.load(); }),
                  Wait::Status::EXIT);
    });
